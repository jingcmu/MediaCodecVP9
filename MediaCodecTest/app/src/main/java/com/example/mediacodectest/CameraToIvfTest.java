...
public class CameraToIvfTest {
    private static final String TAG = "VP9CodecTestBase";
    ...
    private static final String INPUT_FILE = "niklas_640x480";
    private static final int WIDTH = 640;
    private static final int HEIGHT = 480;
    private static final int FRAME_RATE = 30;   // 30fps
    private static final int BITRATE = 800000;   // 1 Mbps

    // parameters for the encoder
    private static final String VP8_MIME = "video/x-vnd.on2.vp9";
    //private static final String VP8_MIME = "video/avc";
    private static final int IFRAME_INTERVAL = 10000;       // 10 seconds between I-frames
    private static final int DURATION_SEC = 100;           // seconds of video
    private static boolean WRITE_IVF = true;
    private static boolean WRITE_YUV = false;				// whether write out the captured YUV
    private static boolean WRITE_DECODED_DATA = false;		// whether write out the decoded YUV
...
//        CameraToIvfWrapper.stopTest();
    ...
    public void swapYV12toI420(byte[] yv12bytes, int width, int height) {
        for (int i = 0; i < (width * height / 4); i++) {
            byte tmp = yv12bytes[width * height + i];
            yv12bytes[width * height + i] = yv12bytes[width * height + (width * height >> 2) + i];
            yv12bytes[width * height + (width * height >> 2) + i] = tmp;
        }
    }
                ...
                if (mInputFrameCount <= mOutputFrameCount + 1) {
                    ...
                    if (inputBufIndex >= 0) {
                        ...
                        if (USE_CLIP) {
                            ...
                            // swapYV12toI420(mFrameData, WIDTH, HEIGHT);
                            ...
                            data = mCameraCallback.getCameraData();
                            swapYV12toI420(data, WIDTH, HEIGHT);
                            mFrameInputTimeMs[mInputFrameCount] = SystemClock.elapsedRealtime();
                            encoderInputBuffers[inputBufIndex].clear();
    ...
    private void prepareEncoder(int width, int height, int bitRate, boolean useSurface) {
        ...
        if (useSurface) {
            ...
            mEncoderColorFormat = CodecCapabilities.COLOR_FormatYUV420Planar; // properties.colorFormat; //CodecCapabilities.COLOR_FormatYUV420Planar; //
