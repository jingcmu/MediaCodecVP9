...
public class DecodeSurfaceTest {
    ...
    protected static final String TAG = "VP9CodecTestBase";
    private static final String VP8_MIME = "video/x-vnd.on2.vp9";
    private static final String VPX_SW_DECODER_NAME = "OMX.google.vp9.decoder";
    private static final String VPX_SW_ENCODER_NAME = "OMX.google.vp9.encoder";
    ...
    protected void decode(
            ...
            boolean useSurface,
            Surface surface,
            VideoRendererIf rendererIf) throws Exception {
        ...
        while (!sawOutputEOS) {
            ...
            while (result >= 0) {
                ...
                if (rendererIf != null && useSurface) {
                    rendererIf.RenderFrame();
                }
                if (bufferInfo.size > 0) {
                    mOutputFrameIndex++;
                }
                if (mOutputFrameIndex < mInputFrameIndex) {
                  result = decoder.dequeueOutputBuffer(bufferInfo, DEFAULT_TIMEOUT_OUTPUT_US);
                } else {
                  break;
                }
    ...
    private void eglSetup(EGLContext sharedContext, int width, int height) {
        ...
        mEGLContext = EGL14.eglCreateContext(mEGLDisplay, configs[0], sharedContext,
                attrib_list, 0);
    ...
    protected void decodeToSurface(
            String inputIvfFilename,
            Surface decSurface,
            VideoRendererIf rendererIf,
            EGLContext sharedContext) throws Exception {
        Log.d(TAG, "Running decoderToSurface on thread id " + Thread.currentThread().getId());
        ...
        if (decSurface == null) {
            // Create EGL context
            eglSetup(sharedContext, frameWidth, frameHeight);
            makeCurrent();
        ...
        MediaCodec decoder = MediaCodec.createByCodecName(properties.codecName);
        decoder.configure(format, decSurface, null, 0);
        decoder.start();
            ...
            if (result >= 0) {
                ...
                if (rendererIf != null) {
                    if (mSurfaceTexture == null) {
                        ...
                        rendererIf.RenderFrame(mTextureID, stMatrix);
                        //rendererIf.RenderFrame(mSurfaceTexture);
                    }
                }
                if (bufferInfo.size > 0) {
                    mOutputFrameIndex++;
    ...
    public void testDecoder(final Surface surface,
            final VideoRendererIf rendererIf,
            final EGLContext sharedContext) throws Exception {
        int frameCount = 10 * FPS;
        final boolean useSurface = true;

        final String encodedIvfFilename = SDCARD_DIR + File.separator + ENCODED_IVF_BASE +
                "_" + WIDTH + "x" + HEIGHT + ".ivf";
        //final String outputYuvFilename = SDCARD_DIR + File.separator + ENCODED_IVF_BASE +
        ...
        mLooperRunner.runCallableNoWait( new Callable<Void>() {
            ...
            public Void call() throws Exception {
                decode(encodedIvfFilename, outputYuvFilename, useSurface, surface, rendererIf);
                //decodeToSurface(encodedIvfFilename, surface, rendererIf, sharedContext);
                getAverageCodecTime();
                return null;
            }
        } );
